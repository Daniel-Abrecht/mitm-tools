...
class TLSStripper(SocksProxy):
  ...
  def remote_connect(self):
    ...
    s = socks.socksocket()
    s.set_proxy(socks.SOCKS5, "127.0.0.1", 2666)
    ...
    with CA.get(sni) as crt:
      ...
      try:
        ...
        with crt.context.wrap_socket(sb, server_side=True) as ssock:
          s = socks.socksocket()
          s.set_proxy(socks.SOCKS5, "127.0.0.1", 3666)
...
if __name__ == '__main__':
  ...
  CA = CertGen('/etc/ssl/CA/CA.pem','/etc/ssl/CA/CA.key')
  with ThreadingTCPServer(('0.0.0.0', 1666), TLSStripper) as server:
